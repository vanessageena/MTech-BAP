...
# # df.to_csv('final_business_clusters_13.csv', index=False)
# # df.to_json('final_business_clusters_13.json', orient='records', lines=True)
...
# plt.plot(range(2, 20), costs, marker='o')
...
print("Starting silhouette_scores calculation")
silhouette_scores = []
for k in range(2, 20):
    print("Calculating silhouette score for k =", k)
    km = KModes(n_clusters=k, init='Huang', n_init=5, verbose=0)
    labels = km.fit_predict(df[category_cols])
    # Matching dissimilarity: count of mismatches
    dist_matrix = pairwise_distances(df[category_cols], metric=lambda x, y: np.sum(x != y))
